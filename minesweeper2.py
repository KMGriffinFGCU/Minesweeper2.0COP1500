...
class Board:
    ...
    def make_new_board(self):
        ...
        # we should construct the list of lists here (a representation of the board)
    ...
    def get_num_neighboring_bombs(self, row, col):
        ...
        # we use these parameters to not go out of bounds
    ...
    def __str__(self):
        ...
        # creates an array of what the user will see
...
# will check if the wire the user chose matches the one randomly assigned to the bomb


def defusebomb(wirecolor, wirechoice):
    if wirecolor == wirechoice:
        return "defused"
    else:
        return "kaboom"
...
def play(dim_size=10, num_bombs=10):
    ...
    while len(board.dug) < board.dim_size ** 2 - num_bombs:
        print(board)
        # 0,0 or 0, 0 or 0,    0 are valid inputs
        ...
        if not safe:
            # dug a bomb
            wirechoice = str(input('You hit a bomb. What wire do you want to cut? red, blue, or green: '))
            if defusebomb(random.choice(wirecolor), wirechoice) == "defused":
                print("bomb defused")
                safe = True
                continue
            else:
    ...
    if safe:
        ...
        print("Game Over, bomb detonated")
